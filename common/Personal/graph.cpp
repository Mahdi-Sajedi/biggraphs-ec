...
// build incoming edge adjacency list , given outgoing edge al
void build_incoming_edges(graph* graph) {
	...
	for (int i = 0; i < num_nodes; i++) {
		...
		int end_node = (i == graph->num_nodes) ? graph->num_edges : graph->outgoing_starts[i + 1];
...
void read_graph_file(std::ifstream& file, int* scratch)
{
	...
	while (!file.eof())
	{
		buffer.clear(); // no need for clearing becuz getline overwrites the buffer in the next line
		std::getline(file, buffer);
